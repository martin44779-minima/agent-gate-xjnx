...
COMMENT ON COLUMN task_main.callback_path IS '回调路径（文根，需与CALLBACK_BASE_URL拼接为完整URL）';
COMMENT ON COLUMN task_main.callback_svc_cd IS '回调时sysHead.svcCd字段值（入参传入）';
COMMENT ON COLUMN task_main.svc_cd IS 'ESB接口编号，与system_id联合定位适配器；直连调用时为空字符串';
...
-- ALTER TABLE task_main ADD COLUMN callback_svc_cd VARCHAR(64);
-- COMMENT ON COLUMN task_main.esb_sys_head IS 'ESB系统头信息（上游传入原始JSON，回调时组装回传）';
-- COMMENT ON COLUMN task_main.cnsmr_sys_no IS '消费者系统号（回调时用于生成cnsmrSrlNo）';
-- COMMENT ON COLUMN task_main.callback_path IS '回调路径（文根，需与CALLBACK_BASE_URL拼接为完整URL）';
-- COMMENT ON COLUMN task_main.callback_svc_cd IS '回调时sysHead.svcCd字段值（入参传入）';
...
-- ============================================
-- 增量迁移：task_main 补充 svc_cd 字段
-- （新库执行 init 脚本时已包含此列，仅已有库需执行）
-- ============================================
...
-- ============================================
-- 说明：adapter_registry 表已废弃
-- 适配器配置改为写入 Redis（key 格式：adapter:{system_id}:{svc_cd}）
-- 通过 POST /api/admin/adapters 接口动态注册，无需操作数据库
-- ============================================
