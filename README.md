# Agent Gate — 智能体统一接入网关

> 面向银行内部各业务部门的 AI 智能体统一接入网关。各业务方通过同一套 REST 接口提交分析请求，网关异步调用 Flowise 智能体完成 AI 处理，并通过 ESB 将结果回调到对应下游系统。首个接入方为反洗钱（AML）合规部门，其他业务部门只需在 Redis 注册适配器配置即可快速接入。

---

## 项目背景

银行内部存在多个业务部门（反洗钱合规、风控、审计等）需要接入 AI 分析能力（基于 Flowise 搭建的智能体 Flow）。若各部门直接对接 Flowise，存在以下问题：

- 各业务系统耦合 Flowise 接口格式，智能体升级时需要逐个修改
- 无统一的认证、限流、重试、防重提交机制
- 无任务生命周期追踪和异步回调能力
- 新业务部门接入成本高，需要重复开发对接逻辑

Agent Gate 作为统一中间层，每个业务部门对应一个 `BizAdapter`（通过 `system_id` 区分），只需在 Redis 中注册适配器的智能体 URL、字段校验规则和回调参数，即可完成接入，无需修改网关核心代码。
